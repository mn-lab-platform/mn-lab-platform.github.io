...
This workflow allows you to import footprint and coordinate measurements from GNSS or Total Station devices and attach them directly to Context, Trench, or Special Find resources in your database.

---

### Step 1: Select Measurement Type

The first step is to determine exactly what type of resource you are importing measurements for. You will be presented with three option cards.
...
*   **Make a Selection:** Click on the card that matches the data you wish to import:
    ...
    *   **Import (O) Special Find Measurements:** For (O) Special Find records.
...
> *Highlight suggestion: Highlight the grid of option cards, specifically drawing a box around the active/selected card (showing the blue border and background).*
...
*   **Search for a Resource:** Use the search input bar to filter the records by name. The list will only display resources that match the category you chose in Step 1.
*   **Select a Resource:** The matching records are displayed as clickable cards. Click on the card representing the specific resource you want to attach the measurements to.
...
### Step 3: Manage Existing Coordinates

Before adding new measurements, the system checks if your selected resource already contains footprint data to prevent accidental overwrites. 

*   **If No Data Exists:** You will see a green checkmark and a message stating: *"The resource has no footprint data yet. You can move on to the next step."* You may safely proceed.
*   **If Data Exists:** A two-panel screen will appear warning you that the resource already has footprint data.
    *   **Select Fields to Overwrite (Left Panel):** You will see a list of existing coordinate fields. Check the box next to any field you wish to overwrite with your new data. (An icon next to each item indicates whether the existing data is currently `valid` or `malformed`).
    *   **Preview Existing Data (Right Panel):** Click on any item in the left list (without checking the box) to display its raw coordinate data in the right-hand text box, allowing you to review it before deciding to overwrite.

> **[IMAGE PLACEHOLDER: Step 3 Manage Existing Coordinates]** 
> *Highlight suggestion: If showing the "Data Exists" state, draw a box around the checkboxes in the left panel and the preview text area in the right panel.*

---

### Step 4: Select Coordinate Reference System (CRS) Type
...
> **[IMAGE PLACEHOLDER: Step 4 Select CRS Type]** 
...
### Step 5: CRS Definition / Coordinate Entry

The interface for this step changes entirely depending on the coordinate system type you selected in Step 4.

#### If you chose "Coordinates are in Local CRS" (Step 4)
...
> **[IMAGE PLACEHOLDER: Step 5 Local CRS - Toggle and Search/Create]** 
...
#### If you chose "Coordinates are in WGS84 CRS" (Step 4)
...
*   **Verify Resource:** A banner at the top confirms the resource you are *"Adding coordinates to"*.
...
*   **Validation:** The system will validate your text. If there is a formatting issue, a red error message will appear. Once valid, you can proceed.

> **[IMAGE PLACEHOLDER: Step 5 WGS84 - Coordinate Entry]** 
> *Highlight suggestion: Highlight the banner at the top, the large coordinate input text area, and specifically draw boxes around the two circular tool buttons on the right side.*

---

### Step 6: Coordinate Entry / Map Display

The interface for this step continues to adapt based on the coordinate system type you selected in Step 4.
...
> **[IMAGE PLACEHOLDER: Step 6 Local CRS - Coordinate Entry]** 
> *Highlight suggestion: Highlight the large coordinate input text area and draw boxes around the two circular tool buttons on the right side.*
...
> **[IMAGE PLACEHOLDER: Step 6 WGS84 - Map Display]** 
...
### Step 7: Map Verification / Verify and Save

Because the order of operations changes based on the coordinate system you selected in Step 4, this step will display one of two screens depending on your path.
...
*   **Change Basemap:** Use the layer control panel located in the top-right corner of the map to switch between available basemaps.
*   **Proceed:** Once you have visually verified the footprint, proceed to the next step to finalize the save.

> **[IMAGE PLACEHOLDER: Step 7 Local CRS - Map Display]** 
...
*   **Save Footprint:** If the data is correct, click the large green **"Add to Resource"** button at the bottom right of the screen. *(If you chose to overwrite data in Step 3, the old data will be replaced now).*
*   **Status:** A loading indicator will appear. Wait for the green success message confirming that the footprint has been permanently attached to your selected resource.

> **[IMAGE PLACEHOLDER: Step 7 WGS84 - Verify and Save]** 
...
### Step 8: Verify and Save / Summary

This step concludes the process for both CRS paths, though the view differs slightly based on your initial choice in Step 4.
...
*   **Save Footprint:** If everything looks correct, click the large green **"Add to Resource"** button at the bottom right of the screen. *(If you chose to overwrite data in Step 3, the old data will be replaced now).*
*   **Proceed:** A loading indicator will appear as the database updates. Once saved, you will move to the final Summary screen.

> **[IMAGE PLACEHOLDER: Step 8 Local CRS - Verify and Save]** 
> *Highlight suggestion: Highlight the dual-pane view showing the GeoJSON and raw text side-by-side, and specifically point an arrow to the large green "Add to Resource" button.*

#### If you chose "Coordinates are in WGS84 CRS" (Step 4) (Workflow Complete)
...
*   **View your Resource:** You can immediately inspect your updated record by clicking the blue **"View Resource"** button. This will open the record in a new browser tab.
*   **Finish:** The workflow is now complete and can be safely closed.

> **[IMAGE PLACEHOLDER: Step 8 WGS84 - Summary]** 
...
*(Note: You will only reach this 9th step if you chose "Coordinates are in Local CRS" in Step 4. The WGS84 path concludes at Step 8).*
...
*   **Confirmation:** You will see a green checkmark and a **"Success!"** message indicating the footprint was securely attached.
*   **View your Resource:** Click the blue **"View Resource"** button to immediately open and inspect your newly updated record in a new browser tab.
*   **Finish:** The workflow is now complete and can be safely closed.

> **[IMAGE PLACEHOLDER: Step 9 Local CRS - Summary]** 
